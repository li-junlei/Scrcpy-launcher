...
**如果你不知道设备的 IP 地址，可以使用扫描功能自动发现。**

1. **开启无线调试**
   - 确保 Android 设备已开启无线调试（通常需要先通过 USB 执行 `有线转无线`）。
2. **执行扫描**
   - 在主界面 IP 输入框旁边点击 `扫描` 图标（放大镜/雷达图标）。
   - 程序将自动扫描真实局域网内的所有设备（自动过滤各种虚拟网卡，兼容 VPN/TUN 模式）。
3. **选择并连接**
   - 在弹出的搜索结果列表中点击你的设备 IP。
   - 程序将自动填入 IP 并尝试发起连接。
...
对于 Android 11 及以上版本，支持使用配对码进行通过无线调试：
...
2. **获取配对码**
   - 点击 `使用配对码配对设备`
   - 记下显示的 IP 地址、端口和 6 位配对码

3. **进行配对**
   - 在 Scrcpy Launcher 点击 `无线配对` 按钮
   - 输入 IP:端口 和配对码
   - 点击 `配对`

4. **连接**
   - 配对成功后，使用无线调试主界面显示的 IP 和端口（非配对端口）进行连接
